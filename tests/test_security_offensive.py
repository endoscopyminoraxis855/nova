...
class TestAuthBruteForce:
    """Test auth rate limiting and lockout."""

    def test_lockout_after_failures(self):
        from app.auth import _record_failure, _check_rate_limit, _auth_failures, _lockouts, _AUTH_MAX_FAILURES
        test_ip = "attacker_offensive_test"
        # Clear state
        ...
        assert "429" in str(exc.value.status_code) or "Too many" in str(exc.value.detail)
...
class TestKGLikeInjection:
    ...
    def test_percent_in_search(self, db):
        ...
        kg.add_fact("charlie", "knows", "dave")
...
class TestTrainingDataPoisoning:
    """Verify training data can't be poisoned via untrusted channels."""

    @pytest.mark.asyncio
    async def test_unauthorized_channel_rejected(self, db):
        from app.core.learning import LearningEngine
        engine = LearningEngine(db)
        with patch("app.core.learning.config") as mock_cfg:
            mock_cfg.TRAINING_DATA_CHANNELS = "api"
            mock_cfg.TRAINING_DATA_PATH = str(Path(db._db_path).parent / "training.jsonl")
            mock_cfg.MAX_TRAINING_PAIRS = 10000
            await engine.save_training_pair(
                query="test", bad_answer="bad", good_answer="good enough to pass quality",
                channel="discord",  # Not in allowed channels
            )
        path = Path(mock_cfg.TRAINING_DATA_PATH)
        if path.exists():
            lines = path.read_text().strip().split("\n")
            assert len(lines) == 0 or all("discord" not in l for l in lines)

    @pytest.mark.asyncio
    async def test_low_confidence_external_rejected(self, db):
        from app.core.learning import LearningEngine
        engine = LearningEngine(db)
        with patch("app.core.learning.config") as mock_cfg:
            mock_cfg.TRAINING_DATA_CHANNELS = "api,discord"
            mock_cfg.TRAINING_DATA_PATH = str(Path(db._db_path).parent / "training2.jsonl")
            mock_cfg.MAX_TRAINING_PAIRS = 10000
            await engine.save_training_pair(
                query="test", bad_answer="bad", good_answer="good enough to pass quality",
                channel="discord",
                confidence=0.3,  # Too low for external channel
            )
        path = Path(mock_cfg.TRAINING_DATA_PATH)
...
class TestConcurrencyAttacks:
    """Try to exploit race conditions."""

    def test_kg_concurrent_supersession_no_data_loss(self, db):
        """Race condition: two threads superseding the same fact simultaneously."""
        from app.core.kg import KnowledgeGraph
        import threading
        ...
        def update(new_company):
            try:
                kg.add_fact("alice", "works_at", new_company)
            except Exception as e:
                errors.append(str(e))

        threads = [
            threading.Thread(target=update, args=("meta",)),
            threading.Thread(target=update, args=("apple",)),
            threading.Thread(target=update, args=("amazon",)),
        ]
        for t in threads:
            t.start()
        for t in threads:
        ...
        assert not errors
        # Should have exactly one current fact
        current = kg.search("alice")
        current_facts = [f for f in current if f.get("subject") == "alice"]
        assert len(current_facts) >= 1

    def test_db_transaction_isolation(self, db):
        """Verify transactions don't see uncommitted data from other threads."""
        import threading
        results = {"thread1": None, "thread2": None}

        def writer():
            with db.transaction() as tx:
                tx.execute("INSERT INTO conversations (id, title) VALUES ('race_test', 'Test')")
                import time; time.sleep(0.1)
                # Transaction not committed yet

        def reader():
            import time; time.sleep(0.05)
            row = db.fetchone("SELECT * FROM conversations WHERE id = 'race_test'")
            results["thread2"] = row
        ...
        # Reader should either see or not see the row, but no crash
        # (SQLite serializes access via WAL + busy_timeout)
...
class TestMassiveInputs:
    ...
    def test_huge_kg_entity_rejected(self, db):
        from app.core.kg import KnowledgeGraph
        kg = KnowledgeGraph(db)
        huge = "x" * 300  # Over 200 char limit
        result = kg.add_fact(huge, "is_a", "test")
        assert not result
